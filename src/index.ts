...
export function apply(ctx: HostContext, config: SessionsDoc): void {
  ...
  ctx.inject<SettingsHost>(['settings'], (sctx) => {
    ...
    ctx.effect(
      ...
      'persona-switcher: settings presentation',
    )
  })
...
/** settings 宿主（行为契约最小面）。0.1.7：无 register/get；声明面来自 Config。 */
interface SettingsHost {
  settings: {
    /** 自带设置页的插件用 auto:false 关掉自动表单；返回 disposer。 */
